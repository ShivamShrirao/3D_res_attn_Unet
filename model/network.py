...
def down_stack(x, filters, blocks, strides=1, frac_dv=0, **kwargs):
    x = AttnBottleneckBlock(filters, dv=int(filters*frac_dv), strides=strides, **kwargs)(x)

    for i in range(1, blocks):
        x = AttnBottleneckBlock(filters, dv=int(filters*frac_dv), **kwargs)(x)
...
def enc_dec(x, frac_dv, stack_args):     # x(64,64,96,80)
    ...
    # do norm act
    y = up_stack(x4,x3, 256, blocks=3, strides=2, frac_dv=frac_dv, **stack_args) # (1024+1024->1024, 4, 6, 5)
    y = up_stack(y, x2, 128, blocks=3, strides=2, frac_dv=frac_dv, **stack_args) # (1024+512->512,16,24,20)
    y = up_stack(y, x1,  64, blocks=2, strides=2, **stack_args)     # ( 512+256->256,32,48,40)
    y = up_stack(y, x ,  32, blocks=2, strides=2, **stack_args)     # ( 256+ 64->128,64,96,80)
    return y
