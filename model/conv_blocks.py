...
class NormAct(layers.Layer):
    def __init__(activation=tf.nn.leaky_relu, norm='gn', gn_grps=8, **kwargs):
...
class ConvNorm(layers.Layer):
    def __init__(filters, kernel_size=3, strides=1, groups=1, deconv=False, padding='same',
        ...
        super().__init__(**kwargs)
        self.lyrs = list()
        self.lyrs.append(layers.Conv3D(filters, kernel_size=kernel_size, strides=strides, padding=padding,
                                       use_bias=not do_norm_act, data_format="channels_first", groups=groups))
        if do_norm_act:
            self.lyrs.append(NormAct(activation=activation, norm=norm, gn_grps=gn_grps))
        if deconv:
...
class AttnBottleneckBlock(layers.Layer):
    ...
    def get_bottleneck(input_shape):
        in_filters = input_shape[1]
        ...
        if self.filters > 0:
            self.groups = compute_factors(in_filters, filters, self.groups) # just to make sure filters divisible by groups
        ...
        x = NormAct(activation=activation, norm=self.norm)(x_s)
